...
<#
.SYNOPSIS
    Opens Arc proxies and MQTT port-forwards for broker observation.

.DESCRIPTION
    Launches background terminal windows for one or both clusters.

    -target both         (default) NUC + ThinkStation
    -target nuc          NUC Arc proxy + MQTT port-forward only
    -target thinkstation ThinkStation Arc proxy only (broker already on localhost:1883 via netsh)

    After running, connect MQTT Explorer to:
...
[CmdletBinding()]
param(
    [ValidateSet("both", "nuc", "thinkstation")]
    [string]$target             = "both",
...
$targetLabel = switch ($target) {
    "both"         { "Both Clusters" }
    "nuc"          { "NUC Only" }
    "thinkstation" { "ThinkStation Only" }
}

Write-Host ""
Write-Host "============================================" -ForegroundColor Magenta
Write-Host "   AIO Observer Setup - $targetLabel" -ForegroundColor Magenta
Write-Host "============================================" -ForegroundColor Magenta
Write-Host ""
...
if ($target -in @("both", "nuc")) {
    if (Test-PortListening -Port $NucProxyPort) {
        Write-Log "Port $NucProxyPort is already in use. A NUC proxy may already be running, or choose a different port." "WARN"
    }
}
if ($target -in @("both", "thinkstation")) {
    if (Test-PortListening -Port $ThinkProxyPort) {
        Write-Log "Port $ThinkProxyPort is already in use. A ThinkStation proxy may already be running, or choose a different port." "WARN"
    }
}
...
if ($target -in @("both", "nuc")) {
    Write-Log "Launching Arc proxy for NUC cluster '$NucClusterName' on port $NucProxyPort..." "STEP"
...
if ($target -in @("both", "thinkstation")) {
    Write-Log "Launching Arc proxy for ThinkStation cluster '$ThinkClusterName' on port $ThinkProxyPort..." "STEP"
...
if ($target -in @("both", "nuc")) {
    Write-Log "Waiting up to ${ProxyWaitSeconds}s for NUC Arc proxy to become ready on port $NucProxyPort..." "STEP"
...
Write-Host "============================================" -ForegroundColor Magenta
Write-Host ""
Write-Host "  $windowCount window(s) opened:" -ForegroundColor White
if ($target -in @("both", "nuc")) {
...
Write-Host ""
Write-Host "  To stop: close all open proxy/forward windows." -ForegroundColor Yellow
Write-Host ""
