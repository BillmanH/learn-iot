...
- **ONVIF Connector `ter:NotAuthorized` on startup (unauthenticated probe)**: The AIO ONVIF connector sends an unauthenticated `GetDeviceInformation` probe before attempting authenticated calls. Cameras that require auth on ALL requests (e.g. Tapo C210 on default settings) will reject this probe, and the connector will mark the endpoint permanently unhealthy without ever trying credentials. **Fix**: Set `fallbackToUsernameTokenAuth: true` in the endpoint's `additionalConfiguration` via the AIO portal: `{"acceptInvalidHostnames":true,"acceptInvalidCertificates":true,"fallbackToUsernameTokenAuth":true}`. This flag is not documented prominently and is not the default. ✅ Confirmed working as of May 2026. See `issues/onvif_connector_bug.md` for full details.
