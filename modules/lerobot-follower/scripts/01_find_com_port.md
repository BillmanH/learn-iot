# How to Find the Follower Arm COM Port on Windows

The SO101 follower arm shows up as a USB Serial (CDC) device on Windows.
You need to know its COM port number before editing `local_config.yaml`.

## Steps

1. Plug in the follower arm USB cable. Make sure the arm is powered on.
...
5. Note the COM number (e.g., `COM4`).

6. Edit `local_config.yaml` in the `lerobot-follower` folder:
   ```yaml
   follower_port: "COM4"   # <-- set this to your COM number
   ```

## Tips

- If both leader and follower are plugged in at the same time, each will have
  its own COM port. Identify which is which by unplugging one at a time.
