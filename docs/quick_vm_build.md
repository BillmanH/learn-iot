...
## 2. Create the VM
```bash
az vm create --resource-group rg-test-vm --name test-vm-01 --image Ubuntu2204 --size Standard_B2s --admin-username azureuser --generate-ssh-keys --output table
```
...
**Cost note**: `Standard_B2s` (~$0.04/hr) is cheap for a short test. The resource group approach guarantees no orphaned resources (disks, NICs, IPs) are left behind billed.
